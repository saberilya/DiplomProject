...
{
    /// <summary>
    /// Логика взаимодействия для PurchaseSellDoc.xaml
    /// </summary>
    public partial class PurchaseSellDoc : Window
    {
        ...
        public PurchaseSellDoc(Cars selectedCar, string clientName, string clientSurname, string clientPhone, string clientSerial, string clientPassport)
        {
            InitializeComponent();
           ...
           ClientName = clientName;
            ...
            brand.Text = $"ПРИМЕР ДОГОВОРА КУПЛИ-ПРОДАЖИ\n\n" +
                                  ...
                                  $"Покупатель: {ClientName} {ClientSurname},номер телефона {ClientPhone} паспорт серии {ClientSerial} № {ClientPassport}, именуемый в дальнейшем \"Покупатель\", с другой стороны, заключили настоящий договор о нижеследующем:\n\n" +
                                  $"1. Предмет договора:\n" +
                                  ...
                                  $"- VIN-номер: {SelectedCar.VIN}\n" +
                                  $"- Цвет: {SelectedCar.Color}\n" +
                                  $"- Пробег: {SelectedCar.KM}\n" +
                                  $"- Стоимость: {SelectedCar.Price}\n\n" +
                                  $"2. Цена и порядок оплаты:\n" +
                                  $"Цена продажи автомобиля составляет {SelectedCar.Price} рублей. Оплата производится Покупателем в полном объеме наличными/безналичным способом на момент передачи автомобиля.\n\n" +
                                  ...
                                  $"Продавец: ____________________          Покупатель: _______________\n\n" +
